...
`docker build . -t qbt-exporter`

To Run local container:

`docker run --name qbt -p 9561:9561 -v ``pwd``/config:/home/gradle qbt-exporter:latest`

This will create a new folder in your working directory named /config which will have the qbittorrent configuration in it
